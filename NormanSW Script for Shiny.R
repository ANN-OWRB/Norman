...
saveRDS(master,"S:\\Monitoring\\STREAMS\\2022 Norman RShiny Calculator\\NormanSW\\NormanSW\\rawabbv2022122.rds")
...
# raw_working <- raw_working %>%
  select(ID, Date, Constituent, Value)
...
# raw_working <- raw_working %>%  
  group_by(ID, Date, Constituent, Value) %>%
...
raw_wide <- raw_wide %>%
  select(ID, Date, Temp_C, 
         DO_mgL, Kjel_N_mgL, NO2NO3_mgL, pH,
         SpC_uScm, TN_mgL, TP_mgL, TsS_mgL,
         Turbidity_NTU)
#saveRDS
saveRDS(raw_wide, "S:\\Monitoring\\STREAMS\\2022 Norman RShiny Calculator\\NormanSW\\NormanSW\\NORMANPOR.rds" )
